...
def permutation_ad_pvalue(obs, null, n_perm=5000, seed=42):
    ...
    for _ in range(n_perm):
        ...
        try:
            ad_p = anderson_ksamp([obs_p.tolist(), null_p.tolist()], variant='midrank', method='asymptotic').statistic
        except Exception:
            ad_p = 0
        if ad_p >= ad_obs:
            count += 1
...
def main():
    ...
    ad_full = anderson_ksamp([obs_arr.tolist(), null_rmin.tolist()], variant='midrank', method='asymptotic')
    ...
    for i, entry in enumerate(obs_entries):
        ...
        ad = anderson_ksamp([reduced.tolist(), null_rmin.tolist()], variant='midrank', method='asymptotic')
        ...
        results.append({'name': entry['name'], 'type': entry['type'], 'rmin': entry['rmin'], 'ad_stat': ad.statistic, 'p': p, 'T': T})
    print()
