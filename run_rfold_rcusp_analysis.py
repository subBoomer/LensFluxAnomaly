...
def run_comparison(label, obs, sim, null_tail_frac=None):
    if len(obs) < 3 or len(sim) < 10:
        ...
        return None
    ad = anderson_ksamp([obs.tolist(), sim.tolist()], variant='midrank', method='asymptotic')
    ...
    print(f'    Tail T={T:.2f}')
    return {'ad_stat': ad.statistic, 'ad_sig': ad.significance_level, 'p_perm': p, 'ks_stat': ks.statistic, 'ks_p': ks.pvalue, 'T': T, 'obs_mean': np.mean(obs), 'obs_std': np.std(obs), 'n_obs': len(obs), 'sim_mean': np.mean(sim), 'sim_std': np.std(sim), 'n_sim': len(sim)}
...
def main():
    ...
    try:
        ...
        for ax, obs, sim, stat, label in [(axes[0], obs_rfold, cdm_rfold, 'R_fold', 'R_fold'), (axes[1], obs_rcusp, cdm_rcusp, 'R_cusp', 'R_cusp')]:
    ...
    np.savez(out_path, obs_rfold=obs_rfold, obs_rcusp=obs_rcusp, cdm_rfold=cdm_rfold, cdm_rcusp=cdm_rcusp, systems=[e['name'] for e in obs_entries], rfold_per_sys=np.array([e['rfold'] for e in obs_entries]), rcusp_per_sys=np.array([e['rcusp'] for e in obs_entries]))
