...
def main():
    ...
    for s in systems:
        ...
        if 'parity' in s and s['parity'] is not None and (len(s['parity']) == 4):
            rfold = compute_rfold(np.array(s['x_arcsec']), np.array(s['y_arcsec']), np.array(s['parity'], dtype=float), np.array(s['fluxes']))
            rcusp = compute_cusp_statistic(np.array(s['x_arcsec']), np.array(s['y_arcsec']), np.array(s['parity']), np.array(s['fluxes']))
            if rfold >= 0:
