...
def main():
    ...
    for i in range(N_PPC):
        ...
        ad = anderson_ksamp([obs_rfold.tolist(), sample.tolist()])
    ...
    obs_ad = anderson_ksamp([obs_rfold.tolist(), cdm_rfold.tolist()]).statistic
    ...
    print('-' * 49)
    for label, p in [('PPC (mean R_fold)', p_mean),
                     ('PPC (tail frac)', p_tail),
                     ('PPC (AD stat)', p_ad),
                     ('PPC (KS stat)', p_ks),
                     ('AD permutation', ad_p_perm)]:
        sig = 'YES' if p < 0.05 else 'no'
        print(f'{label:25s} {p:>10.4f} {sig:>12s}')

    np.savez('outputs/ppc_results.npz',
             ppc_means=ppc_means, ppc_tails=ppc_tails,
             ppc_ad_stats=ppc_ad_stats, ppc_ks_stats=ppc_ks_stats,
             obs_mean=obs_mean, obs_tail=obs_tail,
             p_mean=p_mean, p_tail=p_tail, p_ad=p_ad, p_ks=p_ks)
